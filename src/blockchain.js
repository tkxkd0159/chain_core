...
function getAdjustedDifficulty(aBlockchain) {
  const latestBlock = aBlockchain.latestBlock();
  const prevAdjustmentBlock = aBlockchain.indexWith(
    aBlockchain.length - DIFFICULTY_ADJUSTMENT_INTERVAL
  );
  const timeTaken =
...
function isValidNewBlock(newBlock, previousBlock) {
  if (!isValidBlockStructure(newBlock)) {
    ...
    calculateMerkleRoot(newBlock.data) !== newBlock.header.merkleRoot
  ) {
    ...
    !hashMatchesDifficulty(newBlock.hash(), newBlock.header.difficulty)
  ) {
...
function isValidChain(blockchainToValidate) {
  ...
  for (var i = 1; i < blockchainToValidate.length; i++) {
    if (
      isValidNewBlock(
        blockchainToValidate.indexWith(i),
        tempBlockchain.indexWith(i - 1)
      )
    ) {
...
function replaceChain(newBlockchain) {
  if (
    isReplaceNeeded(blockchain, newBlockchain) &&
